...
const tableRows = [
  ['Scheduling & Dispatch Board', 'Schedule service stops, assign techs, build pool routes, view day summary'],
  ['Pool Count Waiting List', 'See total pools waiting per service type before you build a route'],
  ['Live Route Map', 'Interactive map with all service stops pinned — drag to reorder'],
  ['Water Chemistry Tracking', 'Log chemicals, dosage, readings, and tech per pool'],
  ['Compliance Reports', 'Print-ready chemical treatment reports for any inspector'],
  ...
  ['Package Programs', 'Multi-visit service packages with renewal tracking'],
  ['Client & Lead Management', 'Full CRM for existing customers and open prospects'],
  ['Property Profiles', 'Service history, water chemistry logs, notes, pool size, GPS per address'],
  ...
  ['Unlimited Clients & Pools', 'No caps — 50 accounts or 5,000, same price'],
  ['500 texts/month (US & Canada)', 'Sent and received, 160 characters per text; +$15 per additional 500 after that'],
];

const faqs = [
  { q: "What's the difference between the plans?", a: "Start ($59) suits an owner-operator: 2 users, 2 employees, 1 truck. Pro ($89) adds billing automation and two-way texting, with 3 users and 2 trucks. Crew ($199) removes the caps entirely. Chemical tracking and the time clock are on every plan — you are never locked out of the compliance side." },
  { q: 'What happens after the 14-day free trial?', a: "You'll be prompted to enter a card and continue for from $59/month. If you decide not to continue, your account simply stops — no charges, no cancellation fees. We don't auto-charge without you knowing it's coming." },
  ...
  { q: "What's the SMS pricing?", a: 'In the US and Canada, Start includes 500 texts a month, Pro 1,000, and Crew 2,000 — sent and received. A text is 160 characters; a longer one counts as two. If you go over, you can add blocks of 500 for $15 each. Most businesses with under 300 active properties never hit their limit.' },
  ...
  return (
    <>
      ...
      <div className="hero">
        ...
        <h1>From $59/Month.<br /><span>Every Feature Included.</span></h1>
      ...
      {/* PRICE CARD */}
      <section>
        <div style={{background:'#fff',border:'3px solid var(--orange)',borderRadius:'20px',padding:'52px 48px',maxWidth:'620px',margin:'0 auto',position:'relative',boxShadow:'0 0 0 6px rgba(13,148,136,.1), 0 24px 80px rgba(15,23,32,.12)'}}>
          ...
          <div style={{fontSize:'13px',fontWeight:700,color:'var(--muted)',textTransform:'uppercase',letterSpacing:'1px',marginBottom:'10px'}}>PoolBossPro</div>
          <div style={{fontSize:'72px',fontWeight:800,color:'var(--text)',lineHeight:1}}><sup style={{fontSize:'30px',verticalAlign:'super'}}>$</sup>99</div>
          <div style={{color:'var(--muted)',fontSize:'15px',marginBottom:'8px',marginTop:'6px'}}>per month</div>
          <div style={{fontSize:'16px',color:'var(--text)',fontWeight:600,marginBottom:'32px'}}>Your entire pool service or pool maintenance operation. One price.</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px 24px',marginBottom:'36px',textAlign:'left'}}>
            {['Unlimited Users','Unlimited Clients','Unlimited Pools','Unlimited Leads','Scheduling & Dispatch','Live Route Map','Pool Count Waiting List','Water Chemistry Tracking','Estimates & Invoices','Stripe Payments','Two-Way SMS Inbox','Automated Alerts','Package Programs','Mobile App for Techs','Chemical Compliance Reports','Role-Based Access','Truck Management','Hour Tracking','Review Requests','Dashboard & Reports'].map((item,i) => (
          ...
          <div style={{background:'var(--light-bg)',border:'1.5px solid var(--border)',borderRadius:'10px',padding:'16px 20px',fontSize:'14px',color:'var(--muted)',marginBottom:'28px',lineHeight:1.6}}>
            <strong style={{color:'var(--text)'}}>500 texts/month included, sent and received (US &amp; Canada).</strong> Need more? Add blocks of 500 for <strong style={{color:'var(--text)'}}>$15 each</strong>. Most businesses never exceed the included 500. The SMS credit rolls month to month — you only pay when you actually use it.
          </div>
      ...
      {/* PAIN BAND */}
      <div style={{background:'linear-gradient(135deg,#0f1720 0%,#263445 100%)',padding:'80px 40px',textAlign:'center'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:'24px',maxWidth:'900px',margin:'0 auto'}}>
          {[
            {old:'$149/user/mo',nw:'$59 flat',desc:'Unlimited users. Add every tech and office staff at no extra cost.'},
            ...
            {old:'Generic software',nw:'Built for pools',desc:'Pool count waiting lists, water chemistry logs, pool-specific workflows. Not adapted from a plumbing app.'},
          ].map((c,i) => (
      ...
      <section style={{background:'var(--light-bg)'}}>
        <div style={{textAlign:'center',maxWidth:'900px',margin:'0 auto 48px'}}>
          ...
          <p style={{color:'var(--muted)',fontSize:'17px',maxWidth:'620px',margin:'0 auto'}}>Here&apos;s exactly what you get for from $59/month — nothing hidden, nothing locked behind a higher tier.</p>
        </div>
        <table style={{width:'100%',maxWidth:'900px',margin:'0 auto',borderCollapse:'collapse'}}>
          <thead>
            <tr>
              <th style={{background:'#0f1720',color:'#fff',fontSize:'13px',fontWeight:700,textTransform:'uppercase',letterSpacing:'1px',padding:'14px 20px',textAlign:'left'}}>Feature</th>
              <th style={{background:'#0f1720',color:'rgba(255,255,255,.6)',fontSize:'13px',fontWeight:400,textTransform:'uppercase',letterSpacing:'1px',padding:'14px 20px',textAlign:'left'}}>What it does</th>
              <th style={{background:'#0f1720',color:'#fff',fontSize:'13px',fontWeight:700,textTransform:'uppercase',letterSpacing:'1px',padding:'14px 20px',textAlign:'center'}}>Included</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className="cta-band">
        ...
        <p>14 days of full access — every feature, unlimited everything. See if PoolBossPro fits your operation before you spend a dollar.</p>
        <div className="hero-btns">
      ...
      <div id="sbp-form-1" style={{display:'none',position:'fixed',zIndex:99999,width:'420px',maxWidth:'calc(100vw - 24px)',background:'#fff',borderRadius:'14px',border:'3px solid #0d9488',boxShadow:'0 0 0 4px rgba(13,148,136,.35), 0 16px 60px rgba(0,0,0,.45)',maxHeight:'calc(100vh - 40px)',overflowY:'auto'}}>
        ...
        <div id="sbp1-step2" style={{padding:'24px 28px',display:'none'}}>
          ...
          <div style={{marginBottom:'18px',display:'flex',alignItems:'flex-start',gap:'10px'}}>
            ...
            <label htmlFor="sbp1-agree" style={{fontSize:'13px',color:'#555',cursor:'pointer',lineHeight:1.5}}>I agree to the <a href="https://poolbosspro.com/terms" target="_blank" rel="noreferrer" style={{color:'#0d9488'}}>Terms of Service</a> and <a href="https://poolbosspro.com/privacy-policy" target="_blank" rel="noreferrer" style={{color:'#0d9488'}}>Privacy Policy</a></label>
          </div>
      ...
      <div id="sbp-form-2" style={{display:'none',position:'fixed',zIndex:99999,width:'420px',maxWidth:'calc(100vw - 24px)',background:'#fff',borderRadius:'14px',border:'3px solid #0d9488',boxShadow:'0 0 0 4px rgba(13,148,136,.35), 0 16px 60px rgba(0,0,0,.45)',maxHeight:'calc(100vh - 40px)',overflowY:'auto'}}>
        ...
        <div id="sbp2-step2" style={{padding:'24px 28px',display:'none'}}>
          ...
          <div style={{marginBottom:'18px',display:'flex',alignItems:'flex-start',gap:'10px'}}>
            ...
            <label htmlFor="sbp2-agree" style={{fontSize:'13px',color:'#555',cursor:'pointer',lineHeight:1.5}}>I agree to the <a href="https://poolbosspro.com/terms" target="_blank" rel="noreferrer" style={{color:'#0d9488'}}>Terms of Service</a> and <a href="https://poolbosspro.com/privacy-policy" target="_blank" rel="noreferrer" style={{color:'#0d9488'}}>Privacy Policy</a></label>
          </div>
